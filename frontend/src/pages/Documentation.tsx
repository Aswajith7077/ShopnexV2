...
const ActionBox: React.FC<{
  ...
  return (
    <CustomCard
      className={`mb-12 ${variant === 'gradient' ? 'bg-gradient-to-r from-slate-900 to-slate-800' : ''}`}
    >
      <h3 className="text-xl font-bold mb-4 text-white">{title}</h3>
      <p className="text-slate-300 mb-6">{description}</p>
      <CustomButton onClick={buttonAction}>{buttonText}</CustomButton>
    </CustomCard>
  );
};
...
const ComponentCard: React.FC<ComponentInfo> = ({
  icon,
  title,
  description,
}) => {
  ...
  return (
    <div className="mb-8">
      <CustomCard className="p-4">
        ...
        />
        {caption && (
          <p className="text-sm text-slate-400 mt-2 text-center">{caption}</p>
        )}
      </CustomCard>
    </div>
  );
  ...
  const cmsFeatures: Feature[] = [
    {
      ...
      description:
        'Headlesshost is designed with an API-first philosophy. It provides RESTful APIs that allow developers to fetch content and present it on any platform, enabling flexible content delivery.',
    },
    {
      title: 'Cloud-Based and SaaS:',
      description:
        'Headlesshost is a cloud-based SaaS platform, meaning it is hosted and maintained by Headlesshost, freeing users from the need to manage their own infrastructure. This also allows for automatic updates and scalability.',
    },
    {
      title: 'User-Centric Interface:',
      description:
        'Headlesshost offers a user-friendly web interface where non-technical users can easily create, edit, and manage content. This separation of concerns helps streamline content workflows.',
    },
    {
      title: 'Content Modeling:',
      description:
        'Headlesshost allows users to create custom content models that define the structure of content types (e.g., blog posts, products, landing pages). This flexibility enables businesses to tailor the CMS to their specific needs.',
    },
  ];
    ...
    >
      <div className="mb-8">
        ...
        <p className="text-slate-300 leading-relaxed">
          This template can be used to create a simple documentation website
          using Headlesshost and Next.js. This sample comes with a number of
          pre-built components but can be easily extended with your own custom
          components.
        </p>
      </div>
      ...
      <div className="mb-12">
        ...
        <p className="text-slate-300 mb-8 text-lg leading-relaxed">
          The content in this sample is managed by Headlesshost which is a
          headless content management system (CMS). Here's a breakdown of its
      ...
      <CodeBlock code={codeExample} title="Code Block" language="javascript" />

      <div className="space-y-12">
        <div>
          <h3 className="text-2xl font-bold mb-4 text-white">Creative Work</h3>
          <p className="text-slate-300 mb-6">
            Design is one of the most important aspects of any agency business.
            We believe this is both the plan, and the process and plan with
            which to execute and deliver industry-leading services and solutions
            to our clients.
          </p>
        ...
        <div>
          ...
          <p className="text-slate-300 mb-6">
            Have client in different time zones for the industry and say how it
            goes about a third way to see the changes in their team, along the
            way, we will further the business and manage as we can make the
            process easy and better.
          </p>
        ...
        <div>
          ...
          <p className="text-slate-300">
            Once our timeline team assessed that we would not get the scene
            right in less than the promised timeline, we set about for a new
            team. Across various iterations and rogue events.
          </p>
        </div>
      </div>
    </ContentSection>
  ...
  return (
    ...
    >
      <p className="text-slate-300 text-lg leading-relaxed mb-8">
        Adding pages using this sample site is achieved with a couple of
        distinct steps:
      </p>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="bg-purple-100 text-purple-800 p-3 rounded"
                >
                  <div className="flex items-center space-x-2">
                  ...
                  <p className="text-xs mt-1">
                    Use the name "list" in your context to drag...
                  </p>
                </div>
              ))}
              ...
              <Button
                variant="outline"
                className="w-full border-dashed border-slate-600 text-slate-400 hover:border-slate-500 hover:text-slate-300"
              >
                + INSERT NEW SECTION
              </Button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div>
            ...
            <p className="text-sm text-blue-300">
              Values you can use globally. edit fields
            </p>
          </div>
          <div>
            <h5 className="font-semibold mb-3 text-white">Header</h5>
            <p className="text-sm text-slate-400">
              Edit your header content. edit fields
            </p>
          </div>
          <div>
            <h5 className="font-semibold mb-3 text-white">Footer</h5>
            <p className="text-sm text-slate-400">
              Edit your footer content. edit fields
            </p>
          </div>
        </div>
...
const Sidebar: React.FC<{
  ...
  return (
    <aside
      className={`${isOpen ? 'w-64' : 'w-0'} transition-all duration-300 overflow-hidden bg-slate-950 border-r border-slate-800`}
    >
      <nav className="p-6">
...
const DocumentationPage: React.FC = () => {
  const [activeSection, setActiveSection] =
    React.useState<string>('introduction');
