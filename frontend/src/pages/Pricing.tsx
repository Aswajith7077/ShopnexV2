  ...
  return (
    <div className="space-y-3 mb-8">
      {features.map((feature, index) => (
        <div key={index}>
          {feature === 'CARD RATES FROM' || feature === 'STANDOUT FEATURES' || feature === 'INTEGRATION RATES FROM' ? (
...
const PricingCard = ({ plan, isYearly }: { plan: PricingPlan; isYearly: boolean }) => {
  ...
  return (
    <div className="relative">
      <Card className={`h-full p-6 ${plan.popular ? 'border-cyan-600' : ''}`}>
        {plan.popular && (
        ...
        <div className="mb-6">
          <span className="text-4xl font-bold text-white">{getAdjustedPrice(plan.price)}</span>
  ...
  return (
    <div className="w-full">
      {React.Children.map(children, child =>
        React.cloneElement(child as React.ReactElement<TabsListProps>, { activeTab, onTabChange: setActiveTab })
      )}
    </div>
  )
}
...
const TabsList = ({ children, activeTab, onTabChange }: TabsListProps) => {
  return (
    <div className="inline-flex h-12 items-center justify-center rounded-lg bg-slate-800 p-1 text-slate-400 mb-8">
      {React.Children.map(children, child =>
        React.cloneElement(child as React.ReactElement<TabsTriggerProps>, { activeTab, onTabChange })
      )}
    </div>
  )
}
  ...
  return (
    <button
      className={`inline-flex items-center justify-center whitespace-nowrap rounded-md px-6 py-2 text-sm font-medium transition-all ${
        isActive ? 'bg-slate-950 text-white shadow-sm' : 'hover:bg-slate-700 hover:text-white'
      }`}
...
//   const [activeTab, setActiveTab] = useState('traders')
  ...
  return (
    <div className="min-h-screen bg-black text-white font-[Open_Sans] px-8 py-16">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl font-bold mb-6 text-white">
            unlock <span className="bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">only $1</span> for your first month
          </h2>
          <p className="text-slate-400 mb-8 text-lg">
            Discover the perfect plan for your needs—switch plans as your business grows.
          </p>
