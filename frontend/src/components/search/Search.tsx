...
const Search = () => {
  ...
  const { mutate } = useApiMutation<
    SearchProductRequestType,
    SearchProductResponseType[]
  >(API_ENDPOINTS.SEARCH_PRODUCTS_ENDPOINT, REQUEST_METHODS.POST);
    ...
    <div className="flex flex-row h-full gap-5 p-5">
      <div className="flex flex-col w-[70%]">
        <div className="flex w-full px-5 gap-5">
          <Button
            variant={isSortByPrice ? 'default' : 'outline'}
            onClick={() => setIsSortByPrice(true)}
          >
            Sort by Price
          </Button>
          <Button
            variant={!isSortByPrice ? 'default' : 'outline'}
            onClick={() => setIsSortByPrice(false)}
          >
            Sort by Rating
          </Button>
        </div>
      ...
      <div className="w-[30%] rounded-2xl bg-slate-900 border p-7">Filters</div>
    </div>
  );
};
